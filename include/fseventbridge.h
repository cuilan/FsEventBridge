...
#define LOG_DEBUG(fmt, ...)                                                    \
  LOG_BASE(FEB_LOG_DEBUG, "DEBUG", "\x1b[36m", fmt, ##__VA_ARGS__)
#define LOG_INFO(fmt, ...)                                                     \
  LOG_BASE(FEB_LOG_INFO, "INFO ", "\x1b[32m", fmt, ##__VA_ARGS__)
#define LOG_WARN(fmt, ...)                                                     \
  LOG_BASE(FEB_LOG_WARN, "WARN ", "\x1b[33m", fmt, ##__VA_ARGS__)
#define LOG_ERROR(fmt, ...)                                                    \
  LOG_BASE(FEB_LOG_ERROR, "ERROR", "\x1b[31m", "%s: " fmt, strerror(errno),    \
           ##__VA_ARGS__)
...
// 文件事件模型
typedef struct {
  char path[FEB_MAX_PATH]; // 发生变动的文件路径，非指针，牺牲栈空间，减少了内存碎片和
                           // malloc 失败的风险
